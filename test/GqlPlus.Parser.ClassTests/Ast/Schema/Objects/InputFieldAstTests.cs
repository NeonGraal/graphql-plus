...
public class InputFieldAstTests
  : AstObjectFieldTests<InputFieldAst, IGqlpInputBase>
{
  [Theory, RepeatData(Repeats)]
  public void HashCode_WithDefault(FieldInput input, string def)
      => _checks.HashCode(
        () => new InputFieldAst(AstNulls.At, input.Name, new InputBaseAst(AstNulls.At, input.Type)) { DefaultValue = new FieldKeyAst(AstNulls.At, def) });
  ...
  public void Inequality_BetweenDefaults(FieldInput input, string def1, string def2)
    => _checks.InequalityBetween(def1, def2,
      def => new InputFieldAst(AstNulls.At, input.Name, new InputBaseAst(AstNulls.At, input.Type)) { DefaultValue = new FieldKeyAst(AstNulls.At, def) },
      def1 == def2);

  protected override string AliasesString(FieldInput input, string aliases)
    => $"( !IF {input.Name}{aliases} : {input.Type} )";

  private readonly AstObjectFieldChecks<InputFieldAst, IGqlpInputBase, InputBaseAst> _checks = new(
      (input, objBase) => new(AstNulls.At, input.Name, objBase),
      input => new(AstNulls.At, input.Type),
      arguments => arguments.InputBases());

  internal override IAstObjectFieldChecks<InputFieldAst, IGqlpInputBase> FieldChecks => _checks;
}
