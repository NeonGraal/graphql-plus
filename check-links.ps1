...
"@
  $attrs = [regex]::Matches($content, $attrPattern)
  foreach ($m in $attrs)
  {
...
"@
  $srcs = [regex]::Matches($content, $srcsetPattern)
  foreach ($m in $srcs)
  {
    $val = $m.Groups[1].Value
    if (-not $val)
...
function Is-ExternalLink
{
  ...
  if ($link -match '^(?i)(?:http|https|mailto|tel|javascript|data):')
  {
    return $true
  }
  if ($link -match '^//')
...
{
  Write-Host "Entry file not found: $Entry" -ForegroundColor Red; exit 2
}
if (-not (Test-Path -LiteralPath $Root))
{
  Write-Host "Root directory not found: $Root" -ForegroundColor Red; exit 2
}
...
while ($queue.Count -gt 0)
{
  ...
  {
    $broken.Add([pscustomobject]@{ Source=$file; Link='(file-read)'; Resolved='(unreadable)'; Reason='Cannot read file' }); continue
  }
    ...
    } else
    {
      ...
      if (($ext -eq '.html' -or $ext -eq '.htm') -and (Is-UnderRoot $resolved $rootDirFull))
      {
        if ($MaxDepth -lt 0 -or $depth -lt $MaxDepth)
        {
          ...
          { $queue.Enqueue((New-Object PSObject -Property @{ Path = $resolved; Depth = $depth + 1 }))
          }
        }
      }
    }
