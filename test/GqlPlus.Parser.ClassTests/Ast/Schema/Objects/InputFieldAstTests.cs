...
public class InputFieldAstTests : AstObjectFieldTests<InputFieldAst, InputBaseAst>
{
  ...
  public void HashCode_WithDefault(FieldInput input, string def)
      => _checks.HashCode(
        () => new InputFieldAst(AstNulls.At, input.Name, new(AstNulls.At, input.Type)) { DefaultValue = new FieldKeyAst(AstNulls.At, def) });
  ...
  public void Inequality_BetweenDefaults(FieldInput input, string def1, string def2)
    => _checks.InequalityBetween(def1, def2,
      def => new InputFieldAst(AstNulls.At, input.Name, new(AstNulls.At, input.Type)) { DefaultValue = new FieldKeyAst(AstNulls.At, def) },
      def1 == def2);
