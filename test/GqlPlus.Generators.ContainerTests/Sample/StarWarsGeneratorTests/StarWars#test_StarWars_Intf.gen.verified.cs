...
public interface ItestQueryObject
  : IGqlpInterfaceBase
{
  ItestCharacter Hero { get; }
  ItestCharacter? Call_Hero(testEpisode? parameter);
  ItestHuman Human { get; }
  ItestHuman? Call_Human(string parameter);
  ItestDroid Droid { get; }
  ItestDroid? Call_Droid(ItestId parameter);
  ICollection<ItestCharacter> Characters { get; }
  ICollection<ItestCharacter>? Call_Characters(ICollection<ItestId>? parameter);
}
