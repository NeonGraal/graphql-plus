...
) : IEncoder<Itest_CategoryObject>
{
  ...
  private readonly DeferOne<IEncoder<Itest_TypeRef<test_TypeKind>>> _itest_TypeRef = encoders.EncoderFor<Itest_TypeRef<test_TypeKind>>();
...
{
  private readonly DeferOne<IEncoder<Itest_TypeRefObject<test_TypeKind>>> _itest_TypeRef = encoders.EncoderFor<Itest_TypeRefObject<test_TypeKind>>();
  private readonly DeferOne<IEncoder<TDomainKind>> _domainKind = encoders.EncoderFor<TDomainKind>();
  public Structured Encode(Itest_DomainRefObject<TDomainKind> input)
    => _itest_TypeRef.I.Encode(input)
      .AddEncoded("domainKind", input.DomainKind, _domainKind.I);
}
...
{
  private readonly DeferOne<IEncoder<Itest_ParentTypeObject<test_TypeKind, TItem, TDomainItem>>> _itest_ParentType = encoders.EncoderFor<Itest_ParentTypeObject<test_TypeKind, TItem, TDomainItem>>();
  private readonly DeferOne<IEncoder<TDomainKind>> _domainKind = encoders.EncoderFor<TDomainKind>();
  public Structured Encode(Itest_BaseDomainObject<TDomainKind,TItem,TDomainItem> input)
    => _itest_ParentType.I.Encode(input)
      .AddEncoded("domainKind", input.DomainKind, _domainKind.I);
}
...
) : IEncoder<Itest_DomainItemObject<TItem>>
{
  ...
  public Structured Encode(Itest_DomainItemObject<TItem> input)
    => Structured.Empty()
      .AddEncoded("domain", input.Domain, _itest_Name.I);
}
...
{
  private readonly DeferOne<IEncoder<Itest_DomainRefObject<TDomainKind>>> _itest_DomainRef = encoders.EncoderFor<Itest_DomainRefObject<TDomainKind>>();
  private readonly DeferOne<IEncoder<TValue>> _value = encoders.EncoderFor<TValue>();
  public Structured Encode(Itest_DomainValueObject<TDomainKind,TValue> input)
    => _itest_DomainRef.I.Encode(input)
      .AddEncoded("value", input.Value, _value.I);
}
...
{
  private readonly DeferOne<IEncoder<Itest_BaseDomainItemObject>> _itest_BaseDomainItem = encoders.EncoderFor<Itest_BaseDomainItemObject>();
  public Structured Encode(Itest_DomainTrueFalseObject input)
    => _itest_BaseDomainItem.I.Encode(input)
...
{
  private readonly DeferOne<IEncoder<Itest_DomainItemObject<Itest_DomainTrueFalse>>> _itest_DomainItem = encoders.EncoderFor<Itest_DomainItemObject<Itest_DomainTrueFalse>>();
  public Structured Encode(Itest_DomainItemTrueFalseObject input)
    => _itest_DomainItem.I.Encode(input);
...
{
  private readonly DeferOne<IEncoder<Itest_BaseDomainItemObject>> _itest_BaseDomainItem = encoders.EncoderFor<Itest_BaseDomainItemObject>();
  private readonly DeferOne<IEncoder<Itest_EnumValue>> _itest_EnumValue = encoders.EncoderFor<Itest_EnumValue>();
  public Structured Encode(Itest_DomainLabelObject input)
    => _itest_BaseDomainItem.I.Encode(input)
      .AddEncoded("label", input.Label, _itest_EnumValue.I);
...
{
  private readonly DeferOne<IEncoder<Itest_DomainItemObject<Itest_DomainLabel>>> _itest_DomainItem = encoders.EncoderFor<Itest_DomainItemObject<Itest_DomainLabel>>();
...
{
  private readonly DeferOne<IEncoder<Itest_DomainItemObject<Itest_DomainRange>>> _itest_DomainItem = encoders.EncoderFor<Itest_DomainItemObject<Itest_DomainRange>>();
...
{
  private readonly DeferOne<IEncoder<Itest_DomainItemObject<Itest_DomainRegex>>> _itest_DomainItem = encoders.EncoderFor<Itest_DomainItemObject<Itest_DomainRegex>>();
...
) : IEncoder<Itest_EnumLabelObject>
{
  ...
  public Structured Encode(Itest_EnumLabelObject input)
    => _itest_Aliased.I.Encode(input)
      .AddEncoded("enumType", input.EnumType, _itest_Name.I);
...
) : IEncoder<Itest_EnumValueObject>
{
  ...
  public Structured Encode(Itest_EnumValueObject input)
    => _itest_TypeRef.I.Encode(input)
      .AddEncoded("label", input.Label, _itest_Name.I);
...
{
  private readonly DeferOne<IEncoder<Itest_TypeRefObject<test_SimpleKind>>> _itest_TypeRef = encoders.EncoderFor<Itest_TypeRefObject<test_SimpleKind>>();
...
{
  private readonly DeferOne<IEncoder<Itest_ChildTypeObject<TObjectKind, Itest_ObjBase>>> _itest_ChildType = encoders.EncoderFor<Itest_ChildTypeObject<TObjectKind, Itest_ObjBase>>();
  private readonly DeferOne<IEncoder<Itest_ObjTypeParam>> _itest_ObjTypeParam = encoders.EncoderFor<Itest_ObjTypeParam>();
  private readonly DeferOne<IEncoder<TField>> _field = encoders.EncoderFor<TField>();
  private readonly DeferOne<IEncoder<Itest_ObjAlternate>> _itest_ObjAlternate = encoders.EncoderFor<Itest_ObjAlternate>();
  private readonly DeferOne<IEncoder<Itest_ObjectFor<TField>>> _itest_ObjectFor = encoders.EncoderFor<Itest_ObjectFor<TField>>();
  private readonly DeferOne<IEncoder<Itest_ObjectFor<Itest_ObjAlternate>>> _itest_ObjectFor2 = encoders.EncoderFor<Itest_ObjectFor<Itest_ObjAlternate>>();
  public Structured Encode(Itest_TypeObjectObject<TObjectKind,TField> input)
    => _itest_ChildType.I.Encode(input)
      .AddList("typeParams", input.TypeParams, _itest_ObjTypeParam.I)
      .AddList("fields", input.Fields, _field.I)
      .AddList("alternates", input.Alternates, _itest_ObjAlternate.I)
      .AddList("allFields", input.AllFields, _itest_ObjectFor.I)
      .AddList("allAlternates", input.AllAlternates, _itest_ObjectFor2.I);
}
...
) : IEncoder<Itest_ObjTypeParamObject>
{
  ...
  public Structured Encode(Itest_ObjTypeParamObject input)
    => _itest_Named.I.Encode(input)
      .AddEncoded("constraint", input.Constraint, _itest_TypeRef.I);
...
{
  private readonly DeferOne<IEncoder<Itest_NamedObject>> _itest_Named = encoders.EncoderFor<Itest_NamedObject>();
  private readonly DeferOne<IEncoder<Itest_ObjTypeArg>> _itest_ObjTypeArg = encoders.EncoderFor<Itest_ObjTypeArg>();
  public Structured Encode(Itest_ObjBaseObject input)
    => _itest_Named.I.Encode(input)
      .AddList("typeArgs", input.TypeArgs, _itest_ObjTypeArg.I);
...
) : IEncoder<Itest_TypeParamObject>
{
  ...
  public Structured Encode(Itest_TypeParamObject input)
    => _itest_Described.I.Encode(input)
      .AddEncoded("typeParam", input.TypeParam, _itest_Name.I);
...
{
  private readonly DeferOne<IEncoder<Itest_ObjBase>> _itest_ObjBase = encoders.EncoderFor<Itest_ObjBase>();
  private readonly DeferOne<IEncoder<Itest_Collections>> _itest_Collections = encoders.EncoderFor<Itest_Collections>();
  public Structured Encode(Itest_ObjAlternateObject input)
    => Structured.Empty()
      .AddEncoded("type", input.Type, _itest_ObjBase.I)
      .AddList("collections", input.Collections, _itest_Collections.I);
...
) : IEncoder<Itest_ObjectForObject<TFor>>
{
  ...
  public Structured Encode(Itest_ObjectForObject<TFor> input)
    => Structured.Empty()
      .AddEncoded("objectType", input.ObjectType, _itest_Name.I);
}
...
{
  private readonly DeferOne<IEncoder<Itest_ObjBaseObject>> _itest_ObjBase = encoders.EncoderFor<Itest_ObjBaseObject>();
  private readonly DeferOne<IEncoder<Itest_Modifiers>> _itest_Modifiers = encoders.EncoderFor<Itest_Modifiers>();
...
{
  private readonly DeferOne<IEncoder<Itest_ObjFieldObject<Itest_ObjFieldType>>> _itest_ObjField = encoders.EncoderFor<Itest_ObjFieldObject<Itest_ObjFieldType>>();
