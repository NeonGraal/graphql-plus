...
internal static class SimpleYamlTestHelpers
{
  ...
  private static string[] BlockFirst<T>(this T value, Func<T?, string[]> mapper, string prefix, string indent = "")
  {
    ...
    if (item.Length == 1) {
      ...
      return [prefix + " " + first, .. item[1..].Select(i => indent + i)];
    } else {
      return [prefix, .. item.Select(i => indent + i)];
    }
  }
